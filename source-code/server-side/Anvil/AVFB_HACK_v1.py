...
@anvil.server.http_endpoint("/v1/readText",methods=["POST","OPTIONS"],enable_cors=True)
def readTextFromImage():
  if anvil.server.request.method == 'OPTIONS':
    ...
    actual_image = base64.b64decode(base64_image_str)
    uploaded_image_blob = anvil.BlobMedia(content_type="image/jpeg", content=actual_image, name=f"{session_key}.jpg")
    app_tables.prd_afvb_images.add_row(X_Session_Key=session_key,Image=uploaded_image_blob,Question='OCR')
    
    #Sending to colab
    read_text = anvil.server.call('anvil_conn', uploaded_image_blob)
    print(read_text)
    responsePayload = {'status':'performed OCR on data','session_key':session_key,'answer':read_text}
    return responsePayload
